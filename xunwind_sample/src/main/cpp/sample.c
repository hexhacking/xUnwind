  ...
  if (g_solution == SAMPLE_SOLUTION_FP || g_solution == SAMPLE_SOLUTION_EH) {
    if (!g_signal_interrupted) {
      if (g_solution == SAMPLE_SOLUTION_FP) {
        // FP local unwind
#ifdef __aarch64__
        size_t frames_sz = xunwind_fp_unwind(g_frames, sizeof(g_frames) / sizeof(g_frames[0]),
    ...
    // CFI local unwind
    xunwind_cfi_log(XUNWIND_CURRENT_PROCESS, XUNWIND_CURRENT_THREAD, context, SAMPLE_LOG_TAG,
...
static JNINativeMethod sample_jni_methods[] = {{"nativeTestCfi", "(Z)V", (void *)sample_test_cfi},
                                               {"nativeTestFp", "(ZZ)V", (void *)sample_test_fp},
                                               {"nativeTestEh", "(ZZ)V", (void *)sample_test_eh}};
